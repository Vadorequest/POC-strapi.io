...
module.exports = {
  ...
  fetchAll: (params) => {
    ...
    return Organization.query(function(qb) {
  ...
  edit: async (params, values) => {
    ...
    // Create entry with no-relational data.
    const entry = Organization.forge(params).save(data);
  ...
  search: async (params) => {
    ...
    const searchNoText = Object.keys(Organization._attributes)
      .filter(attribute => attribute !== Organization.primaryKey && !associations.includes(attribute))
      .filter(attribute => !['string', 'text', 'boolean', 'integer', 'decimal', 'float'].includes(Organization._attributes[attribute].type));
